...
class GmpnnCSNetTwosides(GmpnnCSNetDrugBank):
    ...
    def forward(self, gen11,gen22,dis11,batch):
        ...
        tempStr = drug_data.x
        tempSeq = drug_data.xsmpretrain
        torch.save(drug_data.x, 'Structural_view.pt')
        torch.save(drug_data.xsmpretrain, 'Sequential_view.pt')
        ...
        torch.save(view1_specific, 'view1_specific.pt')


        #print(drug_data.x.shape)
        view1_shared = gen11(drug_data.x)
        d1 = dis11(view1_shared.detach())
        tx = dis11(drug_data.xsmpretrain)
        ...
        torch.save(view2_specific, 'view2_specific.pt')


        view2_shared = gen22(drug_data.xsmpretrain)
        d2 = dis11(view2_shared.detach())
        txsm = dis11(drug_data.x)


        view_shared = (view1_shared+view2_shared)/2
        torch.save(view_shared, 'view_shared.pt')
        ...
        tempStr = tempStr[drug_data.batch]
        tempSeq = tempSeq[drug_data.batch]
        ...
        torch.save(scores, 'test_save_tensorP.pt')

        x_j = drug_data.xsmpretrain[node_j_for_pairs]
        x_i = drug_data.xsmpretrain[node_i_for_pairs]
        a1 = global_mean_pool(x_i, drug_pairs.i_indices)
        a2 = global_mean_pool(x_j, drug_pairs.j_indices)
        scores = torch.cat([a1, a2], dim=1)
        torch.save(scores, 'test_save_tensorS.pt')

        tempdata = torch.cat((tempStr,tempSeq),dim = 1)
        xjtemp = tempdata[node_j_for_pairs]
        xitemp = tempdata[node_i_for_pairs]
        a1temp = global_mean_pool(xitemp, drug_pairs.i_indices)
        a2temp = global_mean_pool(xjtemp, drug_pairs.j_indices)
        scorestemp = torch.cat((a1temp,a2temp),dim = 1)
        torch.save(scorestemp,'cat2view.pt')


        xjtemp1 = drug_data.x[node_j_for_pairs]
        xitemp1 = drug_data.x[node_i_for_pairs]
        a1temp1 = global_mean_pool(xitemp1, drug_pairs.i_indices)
        a2temp1 = global_mean_pool(xjtemp1, drug_pairs.j_indices)
        scorestemp = torch.cat((a1temp1, a2temp1), dim=1)
        torch.save(scorestemp, 'sp1.pt')

        xjtemp2 = drug_data.xsmpretrain[node_j_for_pairs]
        xitemp2 = drug_data.xsmpretrain[node_i_for_pairs]
        a1temp2 = global_mean_pool(xitemp2, drug_pairs.i_indices)
        a2temp2 = global_mean_pool(xjtemp2, drug_pairs.j_indices)
        scorestemp = torch.cat((a1temp2, a2temp2), dim=1)
        torch.save(scorestemp, 'sp2.pt')

        xjtemp3 = view_shared[node_j_for_pairs]
        xitemp3 = view_shared[node_i_for_pairs]
        a1temp3 = global_mean_pool(xitemp3, drug_pairs.i_indices)
        a2temp3 = global_mean_pool(xjtemp3, drug_pairs.j_indices)
        scorestemp = torch.cat((a1temp3, a2temp3), dim=1)
        torch.save(scorestemp, 'share.pt')

        scorestemp = torch.cat((a1temp3, a2temp3, a1temp1, a2temp1), dim=1)
        torch.save(scorestemp, 'sp1&share.pt')
        scorestemp = torch.cat((a1temp3, a2temp3, a1temp2, a2temp2), dim=1)
        torch.save(scorestemp, 'sp2&share.pt')
...
class GNNML1(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.edgeweight = nn.Parameter(torch.Tensor(nin1, nin1))
        glorot(self.edgeweight)
        self.emb = torch.nn.Linear(64,1)
    ...
    def forward(self, data):
        ...
        '''alpha = x[edge_index[0]] @ self.edgeweight
        alpha = self.emb(alpha).relu()
