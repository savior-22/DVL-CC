...
def total_num_rel(name):
    ...
    if name.lower() == 'twosides': return 1
    else: raise NotImplementedError


def split_train_valid(data, fold, val_ratio=0.1):
        cv_split = StratifiedShuffleSplit(n_splits=1, test_size=val_ratio, random_state=fold)

        pos_triplets, neg_triplets = data

        #pos_triplets, neg_samples = data
        train_index, val_index = next(iter(cv_split.split(X=pos_triplets, y=pos_triplets[:, 2])))
        train_indexNeg, val_indexNeg = next(iter(cv_split.split(X=neg_triplets, y=neg_triplets[:, 2])))
        train_pos_triplets = pos_triplets[train_index]
        val_pos_triplets = pos_triplets[val_index]
        #print(train_indexNeg)
        #print(val_indexNeg)
        train_neg_triplets = neg_triplets[train_indexNeg]
        test_neg_triplets = neg_triplets[val_indexNeg]
        #train_neg_samples = neg_samples[train_index]
        #test_neg_samples = neg_samples[val_index]

        train_tup = (train_pos_triplets, train_neg_triplets)
        val_tup = (val_pos_triplets, test_neg_triplets)
        '''pos_triplets,neg_triplets = data
        indexpos=[i for i in range(len(pos_triplets))]
        indexneg=[i for i in range(len(neg_triplets))]
        train_index=np.random.shuffle(indexpos)
        train_indexNeg=np.random.shuffle(indexneg)
        train_pos_triplets = pos_triplets[train_index]
        train_neg_triplets = neg_triplets[train_indexNeg]
        train_tup = (train_pos_triplets, train_neg_triplets)
        val_tup=None'''
...
def load_ddi_data_fold(name, fold, batch_size, data_size_ratio, valid_ratio=0.2):
    ...
    def load_split(split_name):
        ...
        triplets = [(d1, d2, r) for d1, d2, r in zip(df['Drug1_ID'], df['Drug2_ID'], df['Y'])]
        for n in range(len(triplets)):
            if triplets[n][2]==1:
                pos_triplets.append(triplets[n])
            else:neg_triplets.append(triplets[n])
    ...
    if dataset_name in ('twosides', 'drugbank'):
        ...
        #print(NUM_EDGE_FEATURES.shape)
        NUM_FEATURES, NUM_EDGE_FEATURES = NUM_FEATURES.shape[1], NUM_EDGE_FEATURES.shape[1]


        all_drug_data = {drug_id: CustomData(x=data[0], edge_index=data[1], edge_feats=data[2], line_graph_edge_index=data[3], xchemfea=data[4] ,xsmiles=data[5], xfp=data[6], xsmvec=data[7], xsmpretrain=data[8])
                    for drug_id, data in all_drug_data.items()}
        for drug_id, data in all_drug_data.items():
            if data['xsmvec'].shape[0]!=94:
                print(drug_id,data)
        ...
        #val_tup = load_split(f'val_fold{fold}')
        #print(len(train_tup[0]))
        #print(len(val_tup[0]))
...
class DrugDataset(Dataset):
    ...
    def __init__(self, pos_neg_tuples, all_drug_data, ratio=1.0, seed=0):
        ...
        '''for pos_item, neg_list in zip(*pos_neg_tuples):
            print(pos_item,neg_list)
            if ((pos_item[0] in self.drug_ids) and (pos_item[1] in self.drug_ids)):
                self.pair_triplets.append((pos_item, neg_list))'''
        #print(len(pos_neg_tuples[0]))
        #print(len(pos_neg_tuples[1]))
        #print(pos_neg_tuples[0][0])
        for i in range(len(pos_neg_tuples[0])):
            self.pair_triplets.append(pos_neg_tuples[0][i])
        for i in range(len(pos_neg_tuples[1])):
            self.pair_triplets.append(pos_neg_tuples[1][i])
        '''for pos_item in zip(*pos_neg_tuples):
            print(pos_item)
            #print( )
            self.pair_triplets.append(pos_item)

        for neg_list in zip(*pos_neg_tuples):
            print(neg_list)
            #print( )
            self.pair_triplets.append(neg_list)'''
        if ratio != 1.0:
...
class TwosidesDataset(DrugDataset):
    ...
    def collate_fn(self, batch):
        ...
        for ind, list in enumerate(batch):
            #print(pos_item)
            #print(neg_list)
            h, t, r = list[:3]
            idx_h, h_num_nodes = self._get_new_batch_id_and_num_nodes(h, old_id_to_new_batch_id, batch_drug_feats)
            ...
            if r=='1':
                ...
                self.set_node_ind_seqs((idx_h, idx_t), 'neg')
                neg_rels.append(int(r))
            #rels.append(int(r))
        #print(rels)
        #print(len(pair_data_neg))
        #print()
        #print(len(pair_data_pos))
        '''for ind, (pos_item, neg_list) in enumerate(batch):
            #print(pos_item)
            #print(neg_list)
            h, t, r = pos_item[:3]
        ...
        #print(batch_drug_data)
        #print(pair_data_pos+pair_data_neg)
        '''print(drug_pairs.i_indices)
        print(drug_pairs.j_indices)'''
        #print(pair_data_pos)
        #print(pair_data_neg)
        #rels = torch.LongTensor(rels)
