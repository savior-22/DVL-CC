...
def run_batch(model, optimizer, data_loader, epoch_i, desc, loss_fn, device, optimizer_G1, optimizer_G2,  optimizer_D1):
        total_loss = 0
        loss_pos = 0
        loss_neg = 0
        probas_pred = []
...
def train(model, train_data_loader, val_data_loader, test_data_loader, loss_fn, optimizer, n_epochs
    ...
    for epoch_i in range(1, n_epochs+1):
        ...
        model.train()
        ## Training
        train_loss, train_metrics = run_batch(model, optimizer, train_data_loader, epoch_i,  'train', loss_fn, device,optimizer_G1, optimizer_G2,  optimizer_D1)
        if scheduler:
            scheduler.step()
        ...
        with torch.no_grad():
            ...
            ## Validation 
            if val_data_loader:
                val_loss , val_metrics = run_batch(model, optimizer, val_data_loader, epoch_i, 'val', loss_fn, device,optimizer_G1, optimizer_G2,  optimizer_D1)
            #if epoch_i==100:
            if test_data_loader:
        ...
        if epoch_i==100:
            with open(r'fold0'+str(fold_i)+'.model', 'wb') as file:
    ...
    '''with open(r'D:\code\co-crystalMultiVisionDanpanbieCopy\co-crystal\fold1.model', 'rb') as file:
        the_model = pickle.load(file)
    the_model.to(device=device)
    test_loss1, test_metrics1 = run_batch(the_model, optimizer, test_data_loader, 1, 'test', loss_fn,
                                          device, optimizer_G1, optimizer_G2, optimizer_D1)
    print_metrics(test_loss1, *test_metrics1)'''

    with open(r'fold0'+str(fold_i)+'.model', 'rb') as file:
        the_model = pickle.load(file)
    the_model.to(device=device)
    print(the_model)
...
# the_model.load_state_dict(torch.load('D:\code\co-crystalChem\co-crystal\my_net.model'))
    ...
    test_loss11, test_metrics11 = run_batch(model, optimizer, test_data_loader, 1, 'test', loss_fn,
                                        device, optimizer_G1, optimizer_G2, optimizer_D1)
    ...
    time11 = np.array(time11)
    np.save('time.npy',time11)
