...
class SpectConv(MessagePassing):
    ...
    """
    #TFT
    def __init__(self, in_channels, out_channels, K=1, selfconn=True, depthwise=False,bias=True, **kwargs):    #depthwise = false
        ...
        self.embedge = nn.Linear(6, self.in_channels)
    ...
    def forward(self, x, edge_index,edge_attr, edge_weight: OptTensor = None,
        ...
        if not self.depthwise:
            ...
            edge_attr = self.embedge(edge_attr).relu()
            if self.selfconn:
            ...
            for i in range(0,enditr):
                #print(Tx_0.shape)
                #print(edge_attr[:,i].shape)

                h = self.propagate(edge_index, x=Tx_0, norm=edge_attr, size=None)
                #h = self.propagate(edge_index, x=Tx_0, norm=edge_attr[:,i], size=None)
                #print(edge_attr[:,i].shape)
                # if self.selfmult:
                #     h=h*Tx_0

                nodeweight = torch.matmul(h, self.weight[i]).sum(-1)
                nodeweight = torch.sigmoid(nodeweight).unsqueeze(-1)
                #print(nodeweight)
                #out = out + h * nodeweight.unsqueeze(-1)
                out = out+ torch.matmul(h, self.weight[i])* nodeweight
        else:
